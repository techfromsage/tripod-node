...
/**
 * Remove all triples where s is the subject
 * @param s
 */
TripleGraph.prototype.removeTriplesAbout = function (s) {
    s = this._uriToAlias(s);
    if (this.hasTriplesAbout(s)) delete this._graphs[s];
};
 ...
 * @return boolean
 */
TripleGraph.prototype.isEmpty = function () {
    return _.isEmpty(this._graphs);
};

/**
 * Does this graph have triples about s?
 * @param s
 * @return boolean
 */
TripleGraph.prototype.hasTriplesAbout = function(s) {
    s = this._uriToAlias(s);
    return (_.has(this._graphs,s));
};

/**
 * Does the graph consider s to be of type t. If t is an array at least once of the types in t must match (OR match)
 * @param s
 * @param t
 * @return boolean
 */
TripleGraph.prototype.isOfType = function(s,t) {
    var aliasedTypes = [], self = this;
    s = this._uriToAlias(s);
    if (this.hasTriplesAbout(s)) {
        if (!_.isArray(t)) {
            aliasedTypes.push(this._uriToAlias(t));
        } else {
            t.forEach(function(_t) {
                aliasedTypes.push(self._uriToAlias(_t));
            });
        }
        return (_.intersection(this.getResourceValues(s,"rdf:type"),aliasedTypes).length>0);
    } else {
        return false;
    }
};

TripleGraph.prototype.hasResourceProperty = function(s,p) {
    if (this.hasTriplesAbout(s)) {
        s = this._uriToAlias(s);
        p = this._uriToAlias(p);
        return _.has(this._graphs[s],p);
    } else {
        return false;
    }
}
